...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        This migration makes sure that we map the old membership
        categories, to the new one. Here is the criteria
        old                           new
        ----------------------------------------------------------------------
        first manager (created the group)  owner
        other managers                     admin
        members                            member          
        """
         
        
        
        for t in orm.Team.objects.iterator():
            print "doing team, %s" % t.slug
            members = t.members.all()
            managers = members.filter(role="manager").order_by("id")
            if managers.count() > 0:
                managers[0].role = 'owner'
                managers[0].save()
                print "\tdid manager to owner, member id %s" % managers[0].pk
                updated = managers.exclude(pk=managers[0].pk).update(role="admin")
                print "\tupdated %s other managers" % (updated)

            # inside migrations you need to use this form to get
            #  access to content types
            content_type =  orm['contenttypes.ContentType'].objects.get(
                app_label="teams", model="team")
            # now make sure we have a valid narrowing, e.g
            # that all members have their privileges applyed to their team
            for m in members:
                if not orm.MembershipNarrowing.objects.filter(member=m).exists():
                    orm.MembershipNarrowing(
                     content_type = content_type, 
                     object_pk = m.pk,
                     member = m,
            added_by=None).save()
                    print "\tcreated narrwoing form member %s" % m.pk

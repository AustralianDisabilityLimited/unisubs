...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        print "Will create default projects for %s teams " % (orm.Team.objects.all().count())
        # this needs to be hardcoded here, since south won't can't be sure
        # about model's attributes later 
        DEFAULT_NAME = "_root"
        created_count = 0
        for t in orm.Team.objects.all():
            try:
                orm.Project.objects.get(team=t)
            except orm.Project.DoesNotExist:
                p = orm.Project(team=t,name=DEFAULT_NAME, modified=datetime.datetime.now())
                p.save()
                created_count += 1
        print "Created %s projects" % (created_count)
    
    def backwards(self, orm):
        # removing the default projects might be a disaster, actually
        pass
